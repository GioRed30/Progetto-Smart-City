...
class Aggregator:
    def __init__(self, config: Dict, logger: logging.Logger):
        ...
        # Totale dei campioni dell'ultimo round: current_weights e' una somma pesata,
        # va divisa per questo valore per ottenere i pesi effettivi del modello.
        self.last_total_size: int = 0
    ...
    def _initialize_weights(self) -> List[np.ndarray]:
        ...
        # I pesi prodotti qui sono quelli che il server distribuisce a tutti i client
        # all'inizio della run: fissare il seme in questo punto rende identico il punto
        # di partenza di due run con la stessa configurazione. Serve al confronto
        # appaiato in chiaro/cifrato, dove altrimenti la differenza di F1 misurerebbe
        # soprattutto l'inizializzazione casuale della testa invece della cifratura.
        # Opzionale: senza "init_seed" nel JSON il comportamento resta quello originale.
        seme = self.config.get('init_seed', None)
        if seme is not None:
            import torch
            torch.manual_seed(int(seme))
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(int(seme))
            np.random.seed(int(seme))
            self.logger.info("Seme di inizializzazione fissato a %d.", int(seme))
    ...
    def aggregate_evaluation_results(self, eval_updates: List[Dict], current_round: int) -> bool:
        ...
        if avg_test_f1 > self.best_f1_score:
            ...
            # current_weights e' la somma pesata sulle dimensioni dei dataset locali:
            # la normalizzazione avviene di norma sui client, qui va rifatta per avere
            # i pesi effettivi del modello globale. In modalita' cifrata current_weights
            # contiene ciphertext e il server non possiede la chiave privata: per
            # costruzione non puo' salvare pesi in chiaro, quindi non si salva nulla.
            if self.config.get('encryption_mode', 'no_encryption') == 'no_encryption':
                if self.last_total_size > 0:
                    self.best_model_weights = [w / self.last_total_size for w in self.current_weights]
                else:
                    self.best_model_weights = list(self.current_weights)
    ...
    def _save_best_weights(self, metrics_filename: str) -> None:
        """
        Serializza i pesi del modello migliore accanto al CSV delle metriche.

        Il nome deriva da quello del CSV sostituendo l'estensione, cosi' la colonna
        'dataframe_path' gia' presente nei risultati permette di risalire ai pesi
        senza aggiungere colonne al CSV condiviso.

        Insieme ai pesi salva i metadati necessari a ricostruire l'architettura,
        altrimenti gli array da soli sarebbero inutilizzabili.
        """
        if not self.best_model_weights:
            modo = self.config.get('encryption_mode', 'no_encryption')
            if modo != 'no_encryption':
                self.logger.info("Modalita' %s: il server non possiede la chiave privata, "
                                 "nessun peso in chiaro da salvare.", modo)
            else:
                self.logger.warning("Nessun peso migliore da salvare.")
            return

        weights_path = os.path.splitext(metrics_filename)[0] + '_weights.npz'
        try:
            np.savez_compressed(
                weights_path,
                **{f'w{i}': w for i, w in enumerate(self.best_model_weights)},
                n_tensori=len(self.best_model_weights),
                ...
                learning_rate=self.config.get('learning_rate', 0),
                num_clients=self.config.get('num_clients', 0),
            )
            n_par = sum(int(np.prod(w.shape)) for w in self.best_model_weights)
            self.logger.info("Salvati i pesi del round migliore (%d) in %s - %d parametri",
                             self.best_round, weights_path, n_par)
        except (OSError, ValueError) as errore:
            self.logger.error("Impossibile salvare i pesi: %s", errore)
    ...
    def save_results(self) -> None:
        ...
        self._save_best_weights(metrics_filename)
        ...
        # Parametri infrastrutturali: non descrivono l'esperimento e non devono finire
        # nel CSV dei risultati, altrimenti ne allargano lo schema rispetto all'header
        # gia' scritto e rendono il file illeggibile.
        keys_to_remove = ['shared_csv_path', 'run_metrics_output_path', 'base_csv_path',
                          ...
                          'torch_threads', 'dataloader_workers', 'cache_dataset', 'init_seed']
