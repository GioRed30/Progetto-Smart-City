...
# Sovrascrivibili da variabile d'ambiente, cosi' da poter lanciare configurazioni
# diverse (es. test_config.json) senza modificare il sorgente.
NUM_PARALLEL_EXECUTIONS = int(os.environ.get('FL_WORKERS', '12'))
# Pausa fra una run e la successiva, in secondi. Serve a far scendere la
# temperatura del pacchetto CPU fra un carico e l'altro: viene applicata solo a
# run conclusa, quando nessun socket e' aperto, quindi non puo' far scadere
# timeout di rete ne' corrompere un'esecuzione in corso.
COOLDOWN_SECONDS = float(os.environ.get('FL_COOLDOWN', '0'))
GRID_SEARCH_CONFIG_PATH = os.environ.get('FL_CONFIG', 'grid_search_config.json')
...
def run_grid_search_worker(
    ...
    while True:
        try:
            ...
            # PyTorch usa per default tutti i core per il parallelismo intra-operazione.
            # Qui ogni worker ospita piu' client concorrenti, ognuno dei quali aprirebbe
            # un pool sull'intera CPU: il risultato e' oversubscription, contesa e CPU
            # inchiodata al 100% anche quando il calcolo pesante e' sulla GPU.
            torch_threads = int(config.get('torch_threads', 0))
            if torch_threads > 0:
                import torch
                torch.set_num_threads(torch_threads)
                print(f"[Worker {worker_id}] torch.set_num_threads({torch_threads})")
            ...
            if COOLDOWN_SECONDS > 0:
                print(f"[Worker {worker_id}] Cooldown termico: pausa di {COOLDOWN_SECONDS:.0f}s.")
...
def main():
    ...
    if os.path.exists(shared_csv_path) and os.path.getsize(shared_csv_path) > 0:
        with open(shared_csv_path, 'r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if 'ResNet' in model_name_from_row or 'GoogLeNet' in model_name_from_row \
                        or 'AlexNet' in model_name_from_row or 'DeiT' in model_name_from_row \
                        or 'ViT' in model_name_from_row:
    ...
    # Le chiavi che iniziano con '_' sono commenti nel JSON: vanno escluse, altrimenti
    # finiscono nel riepilogo della run e aggiungono colonne al CSV dei risultati,
    # disallineandolo rispetto all'header gia' scritto.
        ...
        for model_name, specific_params in model_specific_search_space.items():
            ...
            for hyper_config in generate_configurations(model_base_config, current_search_space):
                ...
                if 'DeiT' in model_name or 'ViT' in model_name:
                    # Vincolo architetturale reale: i checkpoint patch16_224 accettano
                    # solo input 224x224. Qui la forzatura e' corretta.
                    hyper_config['image_size'] = 224
                    hyper_config.setdefault('convnet_hidden1', -1)
                    hyper_config.setdefault('convnet_hidden2', -1)
                elif 'ResNet' in model_name or 'GoogLeNet' in model_name or 'AlexNet' in model_name:
                    # Queste architetture sono completamente convoluzionali fino al
                    # pooling adattivo, quindi accettano qualunque risoluzione: si
                    # rispetta il valore dichiarato nello spazio di ricerca.
